...
S = Solver(templatedir = templatedir, model = "SIAM", symtype = "QSZ", mesh_max = 2.0, mesh_min = 1e-6, mesh_ratio = 1.01)
# Solve Parameters
sp = { "T": T, "Lambda": 2., "Nz": 4, "Tmin": 1e-9, "keep": 1000, "bandrescale": 1.0, "alpha": 0.6}

# Model Parameters
mp = { "U1": U, "eps1": e_f , "B1": B1}
sp["model_parameters"] = mp

# Initialize hybridization function
S.Delta_w['up'] << V**2 * Flat(D)
S.Delta_w['dn'] << V**2 * Flat(D)
